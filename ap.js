...
// ============================================================
// 1. MANEJO DEL MODAL LEGAL (INDEPENDIENTE DEL RESTO)
// ============================================================
...
// ============================================================
// 3. REPRODUCTOR (CORREGIDO Y SIN ERRORES)
// ============================================================
let audioCtx, tracks = [],
    currentIdx = -1,
    isPlaying = false,
    hasStarted = false;
let gainA, gainB, sourceA, sourceB, activeGain = 'A',
    masterGain, mixTimer, analyser;
let currentMode = 'mix';
const MIX_SEGMENT = 144,
    MIX_CROSSFADE = 15,
    PLAYLIST_CROSSFADE = 8;
...
let deckA = { source: null, gain: null, startTime: 0, offset: 0 },
    deckB = { source: null, gain: null, startTime: 0, offset: 0 };
...
function setupMaster() {
    ...
    if (!gainA) { gainA = ac.createGain();
        gainA.gain.value = 0;
        gainA.connect(masterGain); }
    if (!gainB) { gainB = ac.createGain();
        gainB.gain.value = 0;
...
function stopAll() { stopSource(sourceA);
    sourceA = null;
    stopSource(sourceB);
    sourceB = null; if (mixTimer) clearTimeout(mixTimer); }
...
function playSegment(gainNode, trackIndex, startOffset, initialVol) {
    ...
    // 🔥 ONENDED SIEMPRE ACTIVO PARA AMBOS MODOS
    source.onended = () => {
...
function scheduleNext() {
    ...
    if (!currentTrack || !currentDeck.source) {
        setTimeout(() => { if (isPlaying) transitionToNext(); }, 1000);
        return;
    }
...
btnMix.addEventListener('click', (e) => { e.stopPropagation();
    switchMode('mix'); });
btnPlaylist.addEventListener('click', (e) => { e.stopPropagation();
    switchMode('playlist'); });
